...
# (X_train, _), (X_test, _) = mnist.load_data()
    ...
    for file in file_list:
        a = Image.open(file)
        b = a.crop((left, top, right, bottom))
        c = b.resize((50, 50), resample = Image.BICUBIC) # Downsample with bicubic.
        c = c.resize((100, 100), resample = Image.NEAREST) # Upsample with bilinear, to "spoil" the image a bit. I want to have aliasing!
        c = np.array(c)
        x_list.append(c)
